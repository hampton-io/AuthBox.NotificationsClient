...
class NotificationClient {
  ...
  async sendInvitation(email, firstName, lastName, invitationId, code, serviceName, requiresDigipass, selfInvoked, overrides) {
  ...
  async sendSupportRequest(name, email, phone, service, type, message, reference, saUsername) {
    await send('supportrequest_v1', { name, email, phone, service, type, message, reference, saUsername }, this.connectionString);
  }
  ...
  async sendSAPasswordReset(email, firstName, lastName, returnUrl) {
    await send('sapasswordreset_v1', {email, firstName, lastName, returnUrl}, this.connectionString);
  }

  async sendUnmigratedSaUser(email, firstName, lastName, returnUrl) {
    await send('unmigratedsauser_v1', {email, firstName, lastName, returnUrl}, this.connectionString);
  }
}
